      ...
      <nav className="fixed top-0 w-full z-50 bg-surface/80 dark:bg-surface/80 backdrop-blur-xl border-b border-white/10 shadow-sm">
        <div className="flex justify-between items-center max-w-container-max-width mx-auto px-grid-margin-desktop py-6">
          ...
          <div className="hidden md:flex space-x-8">
            ...
            <Link className="font-body-md text-body-md text-on-surface-variant dark:text-on-surface-variant/80 hover:text-primary transition-colors duration-300" to="/why-biomass">Why Biomass</Link>
      ...
      {mobileMenuOpen && (
        <div className="fixed inset-x-0 top-[80px] bg-surface/95 backdrop-blur-xl border-b border-outline-variant/30 shadow-2xl z-40 md:hidden p-6">
          <div className="flex flex-col space-y-4">
            <Link onClick={() => setMobileMenuOpen(false)} className="font-body-md text-lg py-2 border-b border-outline-variant/10 text-primary font-bold pl-2 border-l-2 border-tertiary-container" to="/">Home</Link>
            <Link onClick={() => setMobileMenuOpen(false)} className="font-body-md text-lg py-2 border-b border-outline-variant/10 text-on-surface-variant hover:text-primary" to="/about">About Us</Link>
            <Link onClick={() => setMobileMenuOpen(false)} className="font-body-md text-lg py-2 border-b border-outline-variant/10 text-on-surface-variant hover:text-primary" to="/why-biomass">Why Biomass</Link>
            <Link onClick={() => setMobileMenuOpen(false)} className="font-body-md text-lg py-2 border-b border-outline-variant/10 text-on-surface-variant hover:text-primary" to="/products">Our Products</Link>
            <Link onClick={() => setMobileMenuOpen(false)} className="font-body-md text-lg py-2 border-b border-outline-variant/10 text-on-surface-variant hover:text-primary" to="/industries">Industries</Link>
            <Link onClick={() => setMobileMenuOpen(false)} className="font-body-md text-lg py-2 border-b border-outline-variant/10 text-on-surface-variant hover:text-primary" to="/blogs">Blogs</Link>
            <Link onClick={() => setMobileMenuOpen(false)} className="font-body-md text-lg py-2 border-b border-outline-variant/10 text-on-surface-variant hover:text-primary" to="/contact">Contact</Link>
